...
public class RoundImageView extends ImageView {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mBorderInsideColor != defaultColor && mBorderOutsideColor != defaultColor) {// 定义画两个边框，分别为外圆边框和内圆边框
            radius = getRadiusByBorderNum(2);
            // 画内圆
            drawCircleBorder(canvas, radius + mBorderThickness / 2, mBorderInsideColor);
            // 画外圆
            drawCircleBorder(canvas, radius + mBorderThickness + mBorderThickness / 2, mBorderOutsideColor);
        } else if (mBorderInsideColor != defaultColor && mBorderOutsideColor == defaultColor) {// 定义画一个边框
            radius = getRadiusByBorderNum(1);
            drawCircleBorder(canvas, radius + mBorderThickness / 2, mBorderInsideColor);
        } else if (mBorderInsideColor == defaultColor && mBorderOutsideColor != defaultColor) {// 定义画一个边框
            radius = getRadiusByBorderNum(1);
            drawCircleBorder(canvas, radius + mBorderThickness / 2, mBorderOutsideColor);
        } else {// 没有边框
            radius = getRadiusByBorderNum(0);
        }
        Bitmap roundBitmap = getCroppedRoundBitmap(bitmap == null ? b : bitmap, radius);
        canvas.drawBitmap(roundBitmap, defaultWidth / 2 - radius, defaultHeight / 2 - radius, null);

    }

    public int getRadiusByBorderNum(int borderNum) {
        int radius;
        switch (borderNum) {
            case 0:
    ...
    public Bitmap getCroppedRoundBitmap(Bitmap bmp, int radius) {
        ...
        canvas.drawCircle(scaledSrcBmp.getWidth() / 2f, scaledSrcBmp.getHeight() / 2f,
                scaledSrcBmp.getWidth() / 2f, paint);
    ...
    private void drawCircleBorder(Canvas canvas, int radius, int color) {
        ...
        canvas.drawCircle(defaultWidth / 2f, defaultHeight / 2f, radius, paint);
    }
