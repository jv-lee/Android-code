...
class DiskCacheManager implements Cache {
    ...
    private boolean updateLastModified(File file, long time) {
        if (file.getPath().equals(cacheDir)) {
            return true;
        }
        return file.setLastModified(time) && updateLastModified(file.getParentFile(), time);
    ...
    private long removeLastModifiedFile(File parent) {
        ...
        if (!files.isEmpty()) {
            ...
            boolean result = lastModifyFile.delete();
            if (result) {
                return removedSize;
            }
        }
