...
class CalendarView2Activity : AppCompatActivity(R.layout.activity_calendar_view2) {
    ...
    private val linearContainer by lazy { findViewById<CalendarLinearLayout2>(R.id.linear_container) }
    private val calendarView by lazy { findViewById<CalendarView2>(R.id.calendar_view) }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        calendarView.setOnChangePager(object: CalendarView2.OnChangePager{
        ...
        viewModel.monthLiveData.observe(this, Observer {
            (calendarView.getMonthAdapter() as MonthAdapter2).updateDayStatus(it.position, it.data)
        })

        viewModel.weekLiveData.observe(this, Observer {
            (calendarView.getWeekAdapter() as WeekAdapter2).updateDayStatus(it.position,it.data)
        })
