    ...
    public static Resources setDensity(Resources resources) {
        ...
        if (appDensity == 0) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.ICE_CREAM_SANDWICH) {
                BaseApplication.Companion.getContext().registerComponentCallbacks(new ComponentCallbacks() {
                    @Override
                    public void onConfigurationChanged(Configuration newConfig) {
                        //表示字体发生更改，重新对scaleDensity进行赋值
                        if (newConfig != null && newConfig.fontScale > 0) {
                            appScaleDensity = BaseApplication.Companion.getContext().getResources().getDisplayMetrics().scaledDensity;
                        }
                    }
